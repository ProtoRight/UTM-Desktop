...
//HX711 Pins
  #define HX711_CLK_PIN 7 // reversed pins 6 and 7 to account for perfboard soldering
...
// Global Variables
  ...
  //timing variables
    const unsigned long testingSampleInterval = 200; // ms - Sample rate when machine conducting a test
  ...
  // motor speed variables 
    ...
    float actualTestSpeed = 25; // mm/min of actual crosshead movement - motor jog speed while machine is in test THIS NUMBER IS A PLACEHOLDER
...
if (Serial.available() > 0){ // If there is any data in serial buffer
  ...
  }else {Serial.println("UNKNOWN COMMAND. Please enter: IDLE, RUN_3PT, RUN_T, RAW, CAL, STOP, ZERO, TARE, or JOGSPEED [value] ");}
 }
}
...
void updateRunning_3PT() {
 if (currentState != previousState) {
    ...
    // DRO_ZeroOffset = CurrentDRO; // zero DRO at test start DEPRECATED: replaced by i2C commands below
    Wire.beginTransmission(DRO_ADDR); 
    Wire.write('Z'); 
    Wire.endTransmission();


    scale.tare(); // zero loadcell if desired
  }
  

  if (millis() - lastTestingSampleTime >= testingSampleInterval) {
